...
def generate_confusion_data(
    ...
    # Get class labels based on index mapping
...
def generate_confusion_matrix(
    ...
    for target, prediction in zip(targets, preds):
        ...
        # Count occurrences for each (true label, predicted label) pair
        for true_label, predicted_label in zip(targets, preds):
    ...
    if normalize:
        ...
        row_sums = confusion_matrix.sum(axis=1, keepdims=True)  # Sum per true class
        confusion_matrix = np.divide(confusion_matrix, row_sums, where=row_sums > 0)  # Normalize per class
